...
//	TexNeutModules = TexNeutUtility::ReadModulesFromConfig(argv[1]);	
  ...
  //
	if ( config_macro.size() ) {
		{
			std::ifstream ifs(config_macro.c_str());
			if(!ifs.good()) {
				std::stringstream sst;
				sst << "config macro: \"" << config_macro << "\" does not exist!";
				throw std::invalid_argument(sst.str());
			}
		}
    ...
    // batch mode
		{
			...
			if(!ifs.good()) {
				std::stringstream sst;
				sst << "batch macro: \"" << macro << "\" does not exist!";
				throw std::invalid_argument(sst.str());
			}
		}
